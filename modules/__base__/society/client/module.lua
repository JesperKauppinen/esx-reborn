...
module.RefreshBossHUD = function()
  ...
  -- if player is boss, show business money in hud
...
-- draw $ image and business money in hud if enabled in config
...
-- Remove $ and society money hud element
...
module.OpenBossMenu = function(society, close, options)
  ...
  -- Check if player is boss
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
      ...
      -- Check if options are enabled and if they are add them into table
        ...
        -- Check what option did player select and open that menu
...
-- Employee management menu where u see list of employees and can recruit new one
...
-- list all employees of the society
...
-- List all player online and add option to recruit one.
...
-- List all society grades and let boss choose to change their employees grades.
...
-- Let the boss change salaries of grades.
